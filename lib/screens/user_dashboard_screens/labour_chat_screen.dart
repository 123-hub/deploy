...
class LabourChatScreen extends StatefulWidget {
  static String id = 'FeedbackChatScreen';
  final Room room;

  const LabourChatScreen({super.key, required this.room});

  @override
  _LabourChatScreen createState() => _LabourChatScreen();
}

class _LabourChatScreen extends State<LabourChatScreen> {
  ...
  Future fetchWorkoutFeedbackTillNow() async {
    ...
    var response = await http.get(
      Uri.parse('${Endpoints.labourGetMessages}?room_id=${widget.room.id}'),
    ...
    if (response.statusCode < 299) {
      ...
      Fluttertoast.showToast(
        msg: 'Some problem occured while connecting to the server!',
  ...
  void onStart() async {
    ...
    channel = IOWebSocketChannel.connect(
      '${Endpoints.labourChatStream}?room_id=${widget.room.id}',
      headers: {'Authorization': 'Bearer $token'},
    );
  ...
  void _handleMessageReceived(var message) {
    setState(() {
